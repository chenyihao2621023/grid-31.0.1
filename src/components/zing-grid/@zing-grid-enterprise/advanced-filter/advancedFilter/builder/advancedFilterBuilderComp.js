...
export class AdvancedFilterBuilderComp extends Component {
  ...
  postConstruct() {
    ...
    const {
      showMoveButtons
    } = (_a = this.gridOptionsService.get('advancedFilterBuilderParams')) !== null && _a !== void 0 ? _a : {};
    this.showMove = !!showMoveButtons;
    this.addManagedPropertyListener('advancedFilterBuilderParams', ({
      currentValue
    }) => {
  ...
  createItemComponent(item, focusWrapper) {
    ...
    itemComp.addManagedListener(itemComp, AdvancedFilterBuilderEvents.EVENT_REMOVED, ({
      item
    }) => this.removeItem(item));
    itemComp.addManagedListener(itemComp, AdvancedFilterBuilderEvents.EVENT_VALUE_CHANGED, () => this.validate());
    itemComp.addManagedListener(itemComp, AdvancedFilterBuilderEvents.EVENT_ADDED, ({
      item,
      isJoin
    }) => this.addItem(item, isJoin));
    itemComp.addManagedListener(itemComp, AdvancedFilterBuilderEvents.EVENT_MOVED, ({
      item,
      backwards
  ...
  moveItemUpDown(item, backwards) {
    ...
    const newIndex = this.items.findIndex(({
      filterModel: filterModelToCheck
    }) => filterModelToCheck === filterModel);
    if (newIndex >= 0) {
  ...
  validate() {
    let disableApply = !this.items.every(({
      valid
    }) => valid);
    if (!disableApply) {
  ...
  validateItems() {
    ...
    this.items.forEach(item => {
      ...
      const hasColumn = this.advancedFilterExpressionService.getColumnAutocompleteEntries().find(({
        key
      }) => key === colId);
...
__decorate([RefSelector('eList')], AdvancedFilterBuilderComp.prototype, "eList", void 0);
__decorate([RefSelector('eApplyFilterButton')], AdvancedFilterBuilderComp.prototype, "eApplyFilterButton", void 0);
__decorate([RefSelector('eCancelFilterButton')], AdvancedFilterBuilderComp.prototype, "eCancelFilterButton", void 0);
__decorate([Autowired('filterManager')], AdvancedFilterBuilderComp.prototype, "filterManager", void 0);
__decorate([Autowired('advancedFilterService')], AdvancedFilterBuilderComp.prototype, "advancedFilterService", void 0);
__decorate([Autowired('advancedFilterExpressionService')], AdvancedFilterBuilderComp.prototype, "advancedFilterExpressionService", void 0);
__decorate([Autowired('beans')], AdvancedFilterBuilderComp.prototype, "beans", void 0);
__decorate([PostConstruct], AdvancedFilterBuilderComp.prototype, "postConstruct", null);