...
import { DragSourceType, Component, Autowired, Events, TouchListener, DragAndDropService, PostConstruct, Column, RefSelector, Optional, VirtualList, KeyCode, _ } from "@/components/zing-grid/@zing-grid-community/core/main.js";
export class DropZoneColumnComp extends Component {
  ...
  setupAria() {
    ...
    const ariaInstructions = [[aggFuncName && `${aggFuncName}${aggSeparator}`, name, this.isGroupingZone() && !isSortSuppressed && columnSort && `, ${sortDirection[columnSort]}`].filter(part => !!part).join('')];
    const isFunctionsReadOnly = this.gridOptionsService.get('functionsReadOnly');
