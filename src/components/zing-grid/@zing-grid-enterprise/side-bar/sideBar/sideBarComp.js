...
import { _, Component, Events, ModuleNames, ModuleRegistry, PostConstruct, RefSelector, Autowired, ManagedFocusFeature, KeyCode } from "@/components/zing-grid/@zing-grid-community/core/main.js";
...
export class SideBarComp extends Component {
  ...
  onToolPanelButtonClicked(event) {
    ...
    if (openedItem === id) {
      this.openToolPanel(undefined, 'sideBarButtonClicked');
    } else {
  ...
  setSideBarPosition(position) {
    ...
    this.eventService.dispatchEvent({
      type: Events.EVENT_SIDE_BAR_UPDATED
    });
    return this;
  }
  ...
  createToolPanelAndSideButton(def, initialState) {
    ...
    wrapper.setToolPanelDef(def, {
      initialState,
      onStateUpdated: () => this.eventService.dispatchEvent({
        type: Events.EVENT_SIDE_BAR_UPDATED
      })
    });
...
__decorate([Autowired('filterManager')], SideBarComp.prototype, "filterManager", void 0);
__decorate([Autowired('sideBarService')], SideBarComp.prototype, "sideBarService", void 0);
