...
let EnterpriseMenuFactory = class EnterpriseMenuFactory extends BeanStub {
  ...
  showMenu(column, positionCallback, containerType, defaultTab, restrictToTabs, eventSource) {
    const {
      ...
      currentColumnIndex,
      anchorToElement
    } = this.getMenuParams(column, restrictToTabs, eventSource);
    const closedFuncs = [];
  ...
  isMenuEnabled(column) {
    ...
    const numActiveTabs = isFilterDisabled && tabs.includes(EnterpriseMenu.TAB_FILTER) ? tabs.length - 1 : tabs.length;
...
__decorate([Autowired('filterManager')], EnterpriseMenuFactory.prototype, "filterManager", void 0);
...
export class EnterpriseMenu extends BeanStub {
  ...
  getTabsToCreate() {
    ...
    return this.column.getMenuTabs(EnterpriseMenu.TABS_DEFAULT).filter(tabName => this.isValidMenuTabItem(tabName)).filter(tabName => this.isNotSuppressed(tabName)).filter(tabName => this.isModuleLoaded(tabName));
  }
  ...
  getDefaultMenuOptions() {
    ...
    const allowValueAgg = isPrimary && doingGrouping && allowValue || !isPrimary;
    if (allowPinning) {
