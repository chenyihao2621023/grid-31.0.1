...
import { BaseGridSerializingSession, RowType } from "@/components/zing-grid/@zing-grid-community/csv-export/main.js";
...
export class ExcelSerializingSession extends BaseGridSerializingSession {
  ...
  onNewHeaderGroupingRow() {
    ...
    return {
      onColumn: (columnGroup, header, index, span, collapsibleRanges) => {
        ...
        currentCells.push(Object.assign(Object.assign({}, this.createMergedCell(this.getStyleId(styleIds), this.getDataTypeForValue('string'), header, span)), {
          collapsibleRanges
        }));
      }
    };
  ...
  addRowOutlineIfNecessary(node) {
    ...
    currentRow.hidden = !!node.parent && node.parent.level !== -1 && (collapseAll || this.isAnyParentCollapsed(node.parent));
  }
