...
import { Autowired, Events, SelectionHandleType, _ } from '@/components/zing-grid/@zing-grid-community/core/main.js';
    ...
    const fillValues = (currentValues, col, rowNode, updateInitialSet) => {
      ...
      if (withinInitialRange) {
        ...
        const {
          ...
          sourceCol,
          sourceRowNode
        } = this.processValues(e, currentValues, initialValues, col, rowNode, idx++);
        currentValue = value;
        if (col.isCellEditable(rowNode)) {
          ...
          if (!fromUserFunction) {
            ...
            if (col.getColDef().useValueParserForImport !== false) {
              currentValue = this.valueParserService.parseValue(col, rowNode, sourceCol ? currentValue : _.toStringOrNull(currentValue), cellValue);
            }
          }
  ...
  processValues(event, values, initialValues, col, rowNode, idx) {
    ...
    if (event.altKey || !allNumbers) {
      if (allNumbers && initialValues.length === 1) {
        ...
        return {
          value: parseFloat(_.last(values).value) + 1 * multiplier,
    ...
    return {
      value: _.last(findLineByLeastSquares(values.map(({
        value
      }) => Number(value)))),
      fromUserFunction: false
    };
  ...
  extendVertical(initialPosition, endPosition, isMovingUp) {
    ...
    do {
      ...
      for (let i = 0; i < colLen; i++) {
        ...
        const cellPos = Object.assign(Object.assign({}, rowPos), {
          column
        });
  ...
  reduceVertical(initialPosition, endPosition) {
    ...
    do {
      ...
      for (let i = 0; i < colLen; i++) {
        ...
        const celPos = Object.assign(Object.assign({}, rowPos), {
...
__decorate([Autowired('valueParserService')], FillHandle.prototype, "valueParserService", void 0);
__decorate([Autowired('valueFormatterService')], FillHandle.prototype, "valueFormatterService", void 0);