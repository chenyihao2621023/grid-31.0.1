...
export function createZingChartTheme(chartProxyParams, proxy) {
  ...
  const crossFilteringOverrides = chartProxyParams.crossFiltering ? createCrossFilterThemeOverrides(proxy, chartProxyParams, crossFilterThemeOverridePoint) : undefined;
  ...
  const overrides = [stockTheme ? inbuiltStockThemeOverrides(chartProxyParams, isTitleEnabled()) : undefined, crossFilteringOverrides, gridOptionsThemeOverrides, apiThemeOverrides, formattingPanelOverrides];
  const theme = overrides.filter(v => !!v).reduce((r, n) => ({
    baseTheme: r,
    overrides: n
  }), rootTheme);
...
const STATIC_INBUILT_STOCK_THEME_AXES_OVERRIDES = ALL_AXIS_TYPES.reduce((r, n) => Object.assign(Object.assign({}, r), {
  [n]: {
    title: {
...
export function lookupCustomChartTheme(chartProxyParams, name) {
  ...
  if (!customChartTheme) {
    console.warn(`ZING Grid: no stock theme exists with the name '${name}' and no ` + "custom chart theme with that name was supplied to 'customChartThemes'");
  }
