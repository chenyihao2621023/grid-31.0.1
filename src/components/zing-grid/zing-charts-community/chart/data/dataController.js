...
export class DataController {
    ...
    execute() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const { opts, data, resultCbs, rejects, ids } of merged) {
                const needsValueExtraction = multipleSources ||
                    opts.props.some((p) => {
                        var _a;
    ...
    extractScopedData(id, processedData) {
        ...
        return Object.assign(Object.assign({}, processedData), { data: processedData.data.map((datum) => (Object.assign(Object.assign({}, datum), { datum: extractDatum(datum.datum), values: datum.values.map(extractValues) }))) });
    }
    validateRequests(requested) {
        ...
        for (const [index, request] of requested.entries()) {
            if (index > 0 && request.data.length !== requested[0].data.length && request.opts.groupByData === false) {
                invalid.push(Object.assign(Object.assign({}, request), { error: new Error('all series[].data arrays must be of the same length and have matching keys.') }));
            }
            else {
                valid.push(request);
            }
    ...
    mergeRequested(requested) {
        ...
        const groupMatch = ({ opts, data }) => (gr) => {
            return ((opts.groupByData === false || gr[0].data === data) &&
