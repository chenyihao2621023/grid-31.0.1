...
export class Chart extends Observable {
  ...
  constructor(specialOverrides, resources) {
    ...
    this._destroyFns.push(this.interactionManager.addListener('click', event => this.onClick(event)), this.interactionManager.addListener('dblclick', event => this.onDoubleClick(event)), this.interactionManager.addListener('hover', event => this.onMouseMove(event)), this.interactionManager.addListener('leave', event => this.onLeave(event)), this.interactionManager.addListener('page-left', () => this.destroy()), this.interactionManager.addListener('wheel', () => this.disablePointer()), this.interactionManager.addListener('drag-start', () => this.disablePointer()), this.animationManager.addListener('animation-frame', _ => {
...
__decorate([Validate(UNION(['standalone', 'integrated'], 'a chart mode'))], Chart.prototype, "mode", void 0);