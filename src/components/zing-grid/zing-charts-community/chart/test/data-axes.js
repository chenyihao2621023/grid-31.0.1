...
const countries = ['Ireland', 'Spain', 'United Kingdom', 'France', 'Germany', 'Luxembourg', 'Sweden', 'Norway', 'Italy', 'Greece', 'Iceland', 'Portugal', 'Malta', 'Brazil', 'Argentina', 'Colombia', 'Peru', 'Venezuela', 'Uruguay', 'Belgium'];
...
export const DATA_YOUTUBE_VIDEOS_STATS_BY_DAY_OF_YEAR_LARGE_SCALE = range(100, 300, 5).map(day => {
  return {
    day,
    likes: Math.floor(youtubeRandom2() * 500000),
    subscribes: Math.floor(youtubeRandom2() * 50000),
    comments: Math.floor(youtubeRandom2() * 25000)
  };
});
const youtubeRandom3 = seedRandom(49275017231);
export const DATA_YOUTUBE_VIDEOS_STATS_BY_DATE = dateRange(new Date(2022, 0, 1, 0, 0, 0), new Date(2022, 2, 31, 0, 0, 0)).map(date => {
  return {
    date,
