...
export const DATA_MEAN_SEA_LEVEL = (() => {
  ...
  for (let i = 0; i < 1000; i++) {
    meanSeaLevelData.push({
      time: seaLevelRandom() * 15 + 1998,
      mm: seaLevelRandom() * 100 - 20
    });
  }
...
export const DATA_TIME_MISSING_X = [{
  ...
  x: new Date(1978, 6, 4, 12, 30, 0, 0),
  y1: 37.3,
  y2: 10
}, {
  x: new Date(1979, 6, 4, 12, 30, 0, 0),
  y1: 7.3,
  y2: 5
}];
...
function groupedData(game, country) {
  return {
    labels: [game, country],
