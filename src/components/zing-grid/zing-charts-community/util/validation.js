...
export function Validate(predicate, options = {}) {
    ...
    return addTransformToInstanceProperty((target, property, value) => {
        var _a;
        const context = Object.assign(Object.assign({}, options), { target, property });
...
export const FONT_WEIGHT = OR(UNION(['normal', 'bold', 'bolder', 'lighter'], 'a font weight'), NUMBER.restrict({ min: 1, max: 1000 }));
...
function attachArrayRestrictions(predicate) {
    return Object.assign(predicate, {
        restrict({ length, minLength } = {}) {
            return predicateWithMessage((value) => isArray(value) &&
                (isNumber(length) ? value.length === length : true) &&
                (isNumber(minLength) ? value.length >= minLength : true), isNumber(minLength) && minLength > 0
                ? 'a non-empty array'
                : isNumber(length)
...
function attachObjectRestrictions(predicate) {
    return Object.assign(predicate, {
        restrict(objectType) {
            ...
            return predicateWithMessage((value, ctx) => isInstanceOf(value) || (isObject(value) && isInstanceOf(ctx.target[ctx.property])), (ctx) => { var _a; return (_a = getPredicateMessage(predicate, ctx)) !== null && _a !== void 0 ? _a : 'an object'; });
        },
    });
}
