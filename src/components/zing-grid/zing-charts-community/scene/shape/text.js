...
export class Text extends Shape {
  ...
  static wrapLines(text, maxWidth, maxHeight, textProps, wrapping, overflow) {
    ...
    if (wrapping === 'never') {
      const {
        text,
        truncated
      } = Text.truncateLine(lines[0], maxWidth, measurer, canOverflow ? 'auto' : 'never');
      return {
        lines: text != null ? [text] : undefined,
        truncated
      };
    }
    const wrappedLines = [];
  ...
  static wrap(text, maxWidth, maxHeight, textProps, wrapping, overflow = 'ellipsis') {
    const {
      lines,
      truncated
    } = Text.wrapLines(text, maxWidth, maxHeight, textProps, wrapping, overflow);
    return {
  ...
  static wrapLineSequentially(words, maxWidth, maxHeight, measurer, textProps, wrapping, cumulativeHeight, canOverflow) {
    ...
    const truncateLastLine = () => {
      ...
      const {
        text
      } = Text.truncateLine(lastLine, maxWidth, measurer, 'force');
      if (text == null) {
        return undefined;
      }
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      if (breakWord) {
        ...
        const {
          text
        } = Text.truncateLine(word, maxWidth, measurer, 'force');
        if (text == null) {
          return undefined;
        }
