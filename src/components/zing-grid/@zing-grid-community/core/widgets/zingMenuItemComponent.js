...
import { ZingMenuList } from './zingMenuList';
import { ZingMenuPanel } from './zingMenuPanel';
...
export class ZingMenuItemComponent extends Component {
  constructor(params) {
    super();
    this.params = params;
    this.isActive = false;
  ...
  openSubMenu(activateFirstItem = false) {
    ...
    if (this.params.subMenu instanceof Array) {
      ...
      const childMenu = this.createBean(new ZingMenuList(nextLevel));
      childMenu.setParentComponent(this);
      childMenu.addMenuItems(this.params.subMenu);
      ePopup.appendChild(childMenu.getGui());
      this.addManagedListener(childMenu, ZingMenuItemComponent.EVENT_MENU_ITEM_SELECTED, e => this.dispatchEvent(e));
      ...
      const menuPanel = this.createBean(new ZingMenuPanel(subMenu));
  ...
  onItemSelected(event) {
    ...
    const e = {
      type: ZingMenuItemComponent.EVENT_MENU_ITEM_SELECTED,
  ...
  onItemActivated() {
    const event = {
      type: ZingMenuItemComponent.EVENT_MENU_ITEM_ACTIVATED,
      menuItem: this
    };
  ...
  onMouseEnter() {
    ...
    if (this.params.isAnotherSubMenuOpen()) {
      this.activateTimeoutId = window.setTimeout(() => this.activate(true), ZingMenuItemComponent.ACTIVATION_DELAY);
    } else {
      this.activate(true);
    }
  }
  onMouseLeave() {
    ...
    if (this.isSubMenuOpen()) {
      this.deactivateTimeoutId = window.setTimeout(() => this.deactivate(), ZingMenuItemComponent.ACTIVATION_DELAY);
    } else {
      this.deactivate();
    }
  }
...
ZingMenuItemComponent.EVENT_MENU_ITEM_SELECTED = 'menuItemSelected';
ZingMenuItemComponent.EVENT_MENU_ITEM_ACTIVATED = 'menuItemActivated';
ZingMenuItemComponent.ACTIVATION_DELAY = 80;
__decorate([Autowired('popupService')], ZingMenuItemComponent.prototype, "popupService", void 0);
__decorate([PostConstruct], ZingMenuItemComponent.prototype, "init", null);