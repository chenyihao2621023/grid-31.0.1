...
export class ZipContainer {
  ...
  static getHeader(currentFile, offset) {
    ...
    const header = '\x0A\x00' + (isUTF8 ? '\x00\x08' : '\x00\x00') + '\x00\x00' + decToHex(time, 2) + decToHex(dt, 2) + decToHex(size ? this.getFromCrc32Table(convertedContent) : 0, 4) + decToHex(size, 4) + decToHex(size, 4) + decToHex(utfPath.length, 2) + decToHex(extraFields.length, 2);
    const fileHeader = 'PK\x03\x04' + header + utfPath + extraFields;
    const folderHeader = 'PK\x01\x02' + '\x14\x00' + header + '\x00\x00' + '\x00\x00' + '\x00\x00' + (content ? '\x00\x00\x00\x00' : '\x10\x00\x00\x00') + decToHex(offset, 4) + utfPath + extraFields;
    return {
      fileHeader,
      folderHeader,
      content: convertedContent || ''
  ...
  static buildFolderEnd(tLen, cLen, lLen) {
    ...
    return 'PK\x05\x06' + '\x00\x00' + '\x00\x00' + decToHex(tLen, 2) + decToHex(tLen, 2) + decToHex(cLen, 4) + decToHex(lLen, 4) + '\x00\x00';
  }
