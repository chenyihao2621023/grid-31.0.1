...
export class ErrorBarNode extends _Scene.Group {
  ...
  update(style, formatters, highlighted) {
    ...
    const {
      whiskerStyle,
      capsStyle
    } = this.formatStyles(style, formatters, highlighted);
    const {
      xBar,
      yBar,
      capDefaults
  ...
  updateBBoxes() {
    ...
    const {
      yBar,
      xBar
    } = (_a = this.datum) !== null && _a !== void 0 ? _a : {};
  ...
  nearestSquared(point, maxDistance) {
    ...
    const {
      distanceSquared
    } = BBox.nearestBox(point, bboxes.components);
    return {
      nearest: this,
      distanceSquared
    };
...
export class ErrorBarGroup extends _Scene.Group {
  ...
  nearestSquared(point) {
    const {
      nearest,
      distanceSquared
    } = _Scene.nearestSquaredInContainer(point, this);
