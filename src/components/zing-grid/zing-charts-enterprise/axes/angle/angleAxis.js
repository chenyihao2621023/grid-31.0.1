...
__decorate([Validate(UNION(['fixed', 'parallel', 'perpendicular'], 'a label orientation'))], AngleAxisLabel.prototype, "orientation", void 0);
export class AngleAxis extends _ModuleSupport.PolarAxis {
  ...
  createLabelNodeData(ticks, options, seriesRect) {
    ...
    const labelData = ticks.map((datum, index) => {
      ...
      if (label.formatter) {
        ...
        text = (_a = callbackCache.call(label.formatter, {
          value,
          index
        })) !== null && _a !== void 0 ? _a : '';
      }
      ...
      if (box && options.hideWhenNecessary && !rotation) {
        ...
        if (overflowLeft > pixelError || overflowRight > pixelError) {
          ...
          ({
            text
          } = Text.wrap(text, availWidth, Infinity, label, 'never'));
          if (text === '\u2026') {
            text = '';
            box = undefined;
          }
  ...
  computeLabelsBBox(options, seriesRect) {
    ...
    const textBoxes = this.labelData.map(({
      box
    }) => box).filter(box => box != null);
...
__decorate([ProxyOnWrite('rotation'), Validate(NUMBER.restrict({
  min: 0,
  max: 360
}))], AngleAxis.prototype, "startAngle", void 0);
__decorate([Validate(AND(NUMBER.restrict({
  min: 0,
  max: 720
}), GREATER_THAN('startAngle')), {
  optional: true
