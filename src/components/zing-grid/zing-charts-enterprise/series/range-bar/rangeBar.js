...
export class RangeBarSeries extends _ModuleSupport.AbstractBarSeries {
  ...
  processData(dataController) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const {
        processedData
      } = yield this.requestDataModel(dataController, (_c = this.data) !== null && _c !== void 0 ? _c : [], {
        props: [keyProperty(this, xKey, isContinuousX, {
          id: 'xValue'
        }), valueProperty(this, yLowKey, isContinuousY, {
          id: `yLowValue`
        }), valueProperty(this, yHighKey, isContinuousY, {
          id: `yHighValue`
  ...
  getSeriesDomain(direction) {
    ...
    if (direction === this.getCategoryDirection()) {
      ...
      const fixedYExtent = [yLowExtent[0] > yHighExtent[0] ? yHighExtent[0] : yLowExtent[0], yHighExtent[1] < yLowExtent[1] ? yLowExtent[1] : yHighExtent[1]];
  ...
  createLabelData({
    ...
    const yLowLabel = {
      ...
      text: this.getLabelText(label, Object.assign({
        itemId: 'low',
        value: yLowValue
      }, labelParams), value => isNumber(value) ? value.toFixed(2) : ''),
      itemId: 'low',
      datum,
      series
    };
  ...
  getTooltipHtml(nodeDatum) {
    ...
    const content = yName ? `<b>${sanitizeHtml(xSubheading)}</b>: ${xString}<br>` + `<b>${sanitizeHtml(yLowSubheading)}</b>: ${yLowString}<br>` + `<b>${sanitizeHtml(yHighSubheading)}</b>: ${yHighString}<br>` : `${xString}: ${yLowString} - ${yHighString}`;
    const defaults = {
      title,
      content,
