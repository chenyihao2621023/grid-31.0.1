...
export class BoxPlotSeries extends _ModuleSupport.AbstractBarSeries {
  ...
  processData(dataController) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const {
        processedData
      } = yield this.requestDataModel(dataController, (_b = this.data) !== null && _b !== void 0 ? _b : [], {
        props: [keyProperty(this, xKey, isContinuousX, {
          id: `xValue`
        }), valueProperty(this, minKey, true, {
          id: `minValue`
        }), valueProperty(this, q1Key, true, {
          id: `q1Value`
        }), valueProperty(this, medianKey, true, {
          id: `medianValue`
        }), valueProperty(this, q3Key, true, {
          id: `q3Value`
        }), valueProperty(this, maxKey, true, {
          id: `maxValue`
        }), ...(isContinuousX ? [SMALLEST_KEY_INTERVAL] : []), ...extraProps],
  ...
  getSeriesDomain(direction) {
    ...
    const {
      index,
      def
    } = dataModel.resolveProcessedDataIndexById(this, `xValue`);
  ...
  createNodeData() {
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const xBandWidth = xAxis.scale instanceof _Scale.ContinuousScale ? xAxis.scale.calcBandwidth(smallestDataInterval === null || smallestDataInterval === void 0 ? void 0 : smallestDataInterval.x) : xAxis.scale.bandwidth;
      const domain = [];
      const {
        index: groupIndex,
        visibleGroupCount
      ...
      const nodeData = [];
      const defs = dataModel.resolveProcessedDataDefsByIds(this, ['xValue', 'minValue', 'q1Value', `medianValue`, `q3Value`, `maxValue`]);
      (_a = this.processedData) === null || _a === void 0 ? void 0 : _a.data.forEach(({
        datum,
        keys,
        values
      }) => {
        const {
          ...
          q3Value,
          maxValue
        } = dataModel.resolveProcessedDataDefsValues(defs, {
          keys,
          values
        });
  ...
  getTooltipHtml(nodeDatum) {
    ...
    const contentData = [[xKey, xName, xAxis], [minKey, minName, yAxis], [q1Key, q1Name, yAxis], [medianKey, medianName, yAxis], [q3Key, q3Name, yAxis], [maxKey, maxName, yAxis]];
    const content = contentData.map(([key, name, axis]) => _Util.sanitizeHtml(`${name !== null && name !== void 0 ? name : key}: ${axis.formatDatum(datum[key])}`)).join(title ? '<br/>' : ', ');
    const {
      fill
    } = this.getFormattedStyles(nodeDatum);
  ...
  animateEmptyUpdateReady({
    ...
    const {
      from,
      to
    } = prepareBoxPlotFromTo(isVertical);
  ...
  getFormattedStyles(nodeDatum, highlighted = false) {
    ...
    if (formatter) {
      const formatStyles = callbackCache.call(formatter, Object.assign(Object.assign({
        datum,
        seriesId,
        highlighted
      }, activeStyles), {
