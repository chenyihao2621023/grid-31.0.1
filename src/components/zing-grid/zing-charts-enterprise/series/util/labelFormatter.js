...
__decorate([Validate(TEXT_WRAP)], BaseAutoSizedLabel.prototype, "wrapping", void 0);
__decorate([Validate(OVERFLOW_STRATEGY)], BaseAutoSizedLabel.prototype, "overflowStrategy", void 0);
...
__decorate([Validate(NUMBER)], AutoSizedLabel.prototype, "spacing", void 0);
...
export function generateLabelSecondaryLabelFontSizeCandidates(label, secondaryLabel) {
  const {
    ...
    minimumFontSize: labelMinimumFontSize = labelFontSize
  } = label;
  const {
...
export function formatStackedLabels(labelValue, labelProps, secondaryLabelValue, secondaryLabelProps, {
  padding
}, sizeFittingHeight) {
  var _a, _b;
  const {
  ...
  const minimumHeight = ((_a = labelProps.minimumFontSize) !== null && _a !== void 0 ? _a : labelProps.fontSize) + ((_b = secondaryLabelProps.minimumFontSize) !== null && _b !== void 0 ? _b : secondaryLabelProps.fontSize);
  ...
  return maximumValueSatisfying(0, fontSizeCandidates.length - 1, index => {
    ...
    if (label == null || label.fontSize !== labelFontSize) {
      ...
      const {
        lines: labelLines
      } = Text.wrapLines(labelValue, availableWidth, availableHeight, labelTextSizeProps, labelProps.wrapping, allowTruncation ? labelProps.overflowStrategy : 'hide');
      if (labelLines != null) {
    ...
    if (secondaryLabel == null || secondaryLabel.fontSize !== secondaryLabelFontSize) {
      ...
      const {
        lines: secondaryLabelLines
      } = Text.wrapLines(secondaryLabelValue, availableWidth, availableHeight, secondaryLabelTextSizeProps, secondaryLabelProps.wrapping, allowTruncation ? secondaryLabelProps.overflowStrategy : 'hide');
...
export function formatSingleLabel(value, props, {
  padding
}, sizeFittingHeight) {
  var _a;
  ...
  return maximumValueSatisfying(minimumFontSize, props.fontSize, fontSize => {
    ...
    const {
      lines
    } = Text.wrapLines(value, availableWidth, availableHeight, textSizeProps, props.wrapping, allowTruncation ? props.overflowStrategy : 'hide');
    if (lines == null) {
      return undefined;
    }
