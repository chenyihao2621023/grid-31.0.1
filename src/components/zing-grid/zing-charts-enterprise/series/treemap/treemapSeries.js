...
export class TreemapSeries extends _ModuleSupport.HierarchySeries {
  ...
  squarify(node, bbox, outputBoxes) {
    ...
    const sortedChildrenIndices = Array.from(children, (_, index) => index).filter(index => nodeSize(children[index]) > 0).sort((aIndex, bIndex) => nodeSize(children[bIndex]) - nodeSize(children[aIndex]));
    const childAt = index => {
    ...
    const padding = datum != null ? this.getNodePadding(node, bbox) : {
      top: 0,
      right: 0,
      bottom: 0,
      left: 0
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const labelMeta = Array.from(this.rootNode, node => {
        ...
        if (children.length === 0) {
          ...
          const formatting = formatLabels(labelDatum.label, this.properties.tile.label, labelDatum.secondaryLabel, this.properties.tile.secondaryLabel, {
          ...
          const {
            text
          } = Text.wrap(labelDatum.label, bbox.width - 2 * padding, Infinity, group.label, 'never');
      ...
      const updateLabelFn = (node, text, tag, highlighted) => {
        ...
        if (highlighted) {
          ...
          highlightedColor = !isLeaf ? group.label.color : tag === TextNodeTag.Primary ? tile.label.color : tile.secondaryLabel.color;
        }
