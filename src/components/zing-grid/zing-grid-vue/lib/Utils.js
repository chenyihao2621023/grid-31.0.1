...
export var getZingGridProperties = function () {
  ...
  var DEEP_CHECK_PROPERTIES = ComponentUtil.ALL_PROPERTIES.filter(function (propertyName) {
    return !SHALLOW_CHECK_PROPERTIES.includes(propertyName);
  });
    ...
    rowDataModel: function (currentValue, previousValue) {
      ...
      if (currentValue && previousValue) {
        if (currentValue.length === previousValue.length) {
          if (currentValue.every(function (item, index) {
